...
class MetaDataset(data.Dataset):
    ...
    def __init__(self, root, image_sets, metaclass, img_size, shots=1, shuffle=False, phase=1):
        ...
        self.shot_path  = open(os.path.join(self.root, 'VOCdevkit2007', 'VOC2007', 'ImageSets/Main/shots.txt'), 'w')  # the default saved path
        self.ids = list()
        for (year, name) in image_sets:
            self._year = year
            rootpath = os.path.join(self.root, 'VOCdevkit' + year, 'VOC' + year)
